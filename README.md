...
Each *Combo Input Notify* is assigned a *Combo Key* value, that is mapped to the next move by Move Set Library (`ComboBranches`). This approach allows for reuse of animations in different combo sequences. If the correct input is received while the *Combo Input Notify* is active (and other conditions are met, if such are specified), this notify's *Combo Key* is cached, for *Combo Transition Notify* to handle triggering of the next move later on.  Depending on configuration, *Combo Input Notifies* may override each other.
...
* Standard Notify (Transition point) - a single point of animation, at which a transition between moves is possible. *Combo Key* must to be cached before reaching this point for transition to occur.
